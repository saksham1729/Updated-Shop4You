...
class ProductDetailView(View):
    def get(self, request, pk):
        ...
        if request.user.is_authenticated:
                ...
                recommendations = Product.objects.filter(category=product.category).exclude(id=product.id)[:8]
                user_ratings = ProductRating.objects.filter(user=request.user, product=product)

        else:
            recommendations = Product.objects.filter(category=product.category).exclude(id=product.id)[:8]


        all_reviews = ProductRating.objects.filter(product=product).select_related('user')

            
        return render(request, 'app/productdetail.html', {'product':product, 'recommendations':recommendations,'user_rating':user_ratings , 'item_already_in_cart':item_already_in_cart, 'totalitem':totalitem, 'all_reviews': all_reviews})
...
def cables(request, data=None):
    ...
    if data is None:
        cables = Product.objects.filter(category='C')
    elif data == 'Ambrane' or data == 'pTron' or data == 'AmazonBasics' or data =='boAt':  # Add your actual brand names here
        cables = Product.objects.filter(category='C', brand=data)
    elif data == 'below':
        cables = Product.objects.filter(category='C', discounted_price__lt=500)
    elif data == 'above':
        cables = Product.objects.filter(category='C', discounted_price__gt=500)
    ...
    def post(self, request):
        ...
        if stage == 'send_otp':
            ...
            if phone_form.is_valid():
                ...
                auth_token = '000593200da85d10e7b938064cdd8b0f'
    ...
    if request.method == "GET":
        ...
        if query:
            ...
            for product in products:
                ...
                # ✅ Check for uploaded image first, then fallback to img_link
                if product.product_image:
                    image_url = product.product_image.url
                elif product.img_link:
                    image_url = product.img_link
                else:
                    image_url = '/static/app/images/default.png'  # fallback image
...
def product_3d_view(request):
    return render(request, 'app/3d.html')


@csrf_exempt
def vton_tryon(request):
    result_url = None
    error = None

    if request.method == "POST":
        background_img = request.FILES.get('background_img')
        garm_img = request.FILES.get('garm_img')
        garment_des = request.POST.get('garment_des', '')

        try:
            # Save uploaded files to temp files
            with tempfile.NamedTemporaryFile(delete=False) as bg_tmp, tempfile.NamedTemporaryFile(delete=False) as garm_tmp:
                for chunk in background_img.chunks():
                    bg_tmp.write(chunk)
                for chunk in garm_img.chunks():
                    garm_tmp.write(chunk)
                bg_tmp.flush()
                garm_tmp.flush()

                client = Client("yisol/IDM-VTON")
                result = client.predict(
                    dict={"background": file(bg_tmp.name), "layers": [], "composite": None},
                    garm_img=file(garm_tmp.name),
                ...
                # result is usually a URL or file path to the output image
                result_url = result if isinstance(result, str) else None

        except Exception as e:
            error = str(e)

    return render(request, "app/vton_tryon.html", {"result_url": result_url, "error": error})
...
def expense_tracker(request):
    purchases = OrderPlaced.objects.filter(user=request.user).select_related('product').order_by('-ordered_date')
    period = request.GET.get('period', 'month')  # 'day', 'week', or 'month'
    today = timezone.now().date()
    


    if period == 'day':
        # Only today's purchases
        purchases = purchases.filter(ordered_date__date=today)
        data = purchases.annotate(day=TruncDay('ordered_date')).values('day').annotate(total=Sum('product__discounted_price')).order_by('day')
        labels = [d['day'].strftime('%Y-%m-%d') for d in data]
        totals = [float(d['total']) for d in data]
        total_price = sum([p.product.discounted_price for p in purchases])
    elif period == 'week':
        # Last 7 days
        week_ago = today - timedelta(days=6)
        purchases = purchases.filter(ordered_date__date__gte=week_ago)
        data = purchases.annotate(day=TruncDay('ordered_date')).values('day').annotate(total=Sum('product__discounted_price')).order_by('day')
        labels = [d['day'].strftime('%Y-%m-%d') for d in data]
        totals = [float(d['total']) for d in data]
        total_price = sum([p.product.discounted_price for p in purchases])
    else:
        data = purchases.annotate(month=TruncMonth('ordered_date')).values('month').annotate(total=Sum('product__discounted_price')).order_by('month')
        labels = [d['month'].strftime('%Y-%m') for d in data]
        totals = [float(d['total']) for d in data]
        total_price = sum([p.product.discounted_price for p in purchases])

    product_labels = [p.product.title for p in purchases]
    product_prices = [float(p.product.discounted_price) for p in purchases]


    return render(request, 'app/expense_tracker.html', {
        'purchases': purchases,
        'labels': labels,
        'totals': totals,
        'period': period,
...
from .utils.pose_estimator import process_image_and_recommend_size

@login_required
def recommend_size_view(request, product_id):
    product = get_object_or_404(Product, id=product_id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user_height = form.cleaned_data['height_cm']
            customer = Customer.objects.filter(user=request.user).first()
            if customer:
                    customer.height_cm = user_height
                    customer.save()
            
            recommended_size ,original_image_url,annotated_image_url, pants_size = process_image_and_recommend_size(image, user_height)
            RecommendationLog.objects.create(user=request.user, product=product)
    else:
        form = MeasurementUploadForm()

    return render(request, 'app/recommend_size.html', {
        'form': form,
        'product': product,
